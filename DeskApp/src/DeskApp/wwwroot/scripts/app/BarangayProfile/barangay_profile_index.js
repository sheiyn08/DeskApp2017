   ...
   .then(function (response) {
    ...
    $scope.search = function (page) {
        ...
        if (($scope.data.filter_by_recent_edit) && (!$scope.data.filter_by_recent_add)) {
            ...
            //alert("Recently edited is checked!");

            $.post('/api/offline/v1/barangay_profile/get_recently_edited', $scope.data).success(function (value) {
                $scope.loading = false;
                ...
                angular.forEach($scope.Items, function (record) {
                    $http.get('/api/exists/record_attachment?id=' + record.brgy_profile_id)
            ...
            $.post('/api/offline/v1/barangay_profile/get_recently_added', $scope.data).success(function (value) {
                $scope.loading = false;
            ...
            $.post('/api/offline/v1/barangay_profile/get_recently_edited_and_added', $scope.data).success(function (value) {
                $scope.loading = false;
            ...
            $.post('/api/offline/v1/barangay_profile/get_dto', $scope.data).success(function (value) {
                $scope.loading = false;
    ...
    $scope.syncPost = function () {
        ...
        if ($scope.list_of_selected_items.length > 0) {
            ...
            for (i = 0; i < $scope.list_of_selected_items.length; i++) {
                var url = '/api/offline/v1/barangay_profile/post/selected_items_to_sync?brgy_profile_id='
                        + items_selected[i].brgy_profile_id + "&push_status_id=" + 5;
                $scope.isAddingItem = true;

                $http.post(url).success(function (data) {
                    $scope.isAddingItem = false;
                    //inner http.post -- actual sync:
                    $http.post('/sync/post/barangay_profile?username=' + username + "&password=" + password + "&record_id=").success(function (data) {
                        $scope.false = true;
                        $scope.isSearching = false;
                        location.reload();
                        $mdDialog.cancel();
