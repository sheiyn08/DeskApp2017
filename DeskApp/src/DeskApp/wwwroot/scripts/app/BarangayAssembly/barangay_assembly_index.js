...
.then(function (response) { $scope.enrollment_id_options = response.data; });
    ...
    $scope.search = function (page) {
        ...
        if (($scope.data.filter_by_recent_edit) && (!$scope.data.filter_by_recent_add)) {
            $.post('/api/offline/v1/barangay_assembly/get_recently_edited', $scope.data).success(function (value) {
                $scope.loading = false;
            ...
            $.post('/api/offline/v1/barangay_assembly/get_recently_added', $scope.data).success(function (value) {
                $scope.loading = false;
                $scope.TotalUnAuthorized = value.TotalUnAuthorized;
                $scope.TotalSync = value.TotalSync;
                $scope.page = value.Page;
        ...
        if (($scope.data.filter_by_recent_edit) && ($scope.data.filter_by_recent_add)) {
            $.post('/api/offline/v1/barangay_assembly/get_recently_edited_and_added', $scope.data).success(function (value) {
                $scope.loading = false;
                $scope.TotalUnAuthorized = value.TotalUnAuthorized;
                $scope.TotalSync = value.TotalSync;
                $scope.page = value.Page;
            ...
            $.post('/api/offline/v1/barangay_assembly/get_dto', $scope.data).success(function (value) {
                $scope.loading = false;
    ...
    $scope.createNew = function () {
        ...
        window.location.href = "/Entry/BarangayAssembly";
    }
    ...
    $scope.syncPost = function () {
        ...
        if ($scope.list_of_selected_items.length > 0) {
            ...
            for (i = 0; i < $scope.list_of_selected_items.length; i++) {
                var url = '/api/offline/v1/barangay_assembly/post/selected_items_to_sync?brgy_assembly_id='
                        + items_selected[i].brgy_assembly_id + "&push_status_id=" + 5;
