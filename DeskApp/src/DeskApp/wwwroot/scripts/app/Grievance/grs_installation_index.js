...
$scope.delete = function (removeitem) {
        ...
        if (ask == true) {
            $.post('/api/delete/grievance_installation?id=' + removeitem.grs_installation_id).success(function (value) {
                $scope.loading = false;
...
.then(function (response) { $scope.enrollment_id_options = response.data; });
    ...
    $scope.search = function (page) {
        ...
        if (($scope.data.filter_by_recent_edit) && (!$scope.data.filter_by_recent_add)) {
            $.post('/api/offline/v1/grs_installation/get_recently_edited', $scope.data).success(function (value) {
                $scope.loading = false;
                ...
                angular.forEach($scope.Items, function (record) {
                    $http.get('/api/exists/record_attachment?id=' + record.grs_installation_id)
            ...
            $.post('/api/offline/v1/grs_installation/get_recently_added', $scope.data).success(function (value) {
                $scope.loading = false;
            ...
            $.post('/api/offline/v1/grs_installation/get_recently_edited_and_added', $scope.data).success(function (value) {
                $scope.loading = false;
            ...
            $.post('/api/offline/v1/grs_installation/get_dto', $scope.data).success(function (value) {
                $scope.loading = false;
    ...
    $scope.search();
     ...
     $http.get('/api/table_name?name=grievance_installation')
   .then(function (response) {
    ...
    $scope.createNew = function () {
           ...
           window.location.href = "/Entry/GrievanceInstallation";
    }
    ...
    $scope.syncPost = function () {
        ...
        if ($scope.list_of_selected_items.length > 0) {
            ...
            for (i = 0; i < $scope.list_of_selected_items.length; i++) {
                var url = '/api/offline/v1/grs_installation/post/selected_items_to_sync?grs_installation_id='
                        + items_selected[i].grs_installation_id + "&push_status_id=" + 5;
                $scope.isAddingItem = true;

                $http.post(url).success(function (data) {
                    $scope.isAddingItem = false;
                    //inner http.post -- actual sync:
                    $http.post('/sync/post/grs_installation?username=' + username + "&password=" + password + "&record_id=").success(function (data) {
                        $scope.false = true;
                        $scope.isSearching = false;
                        location.reload();
                        $mdDialog.cancel();
