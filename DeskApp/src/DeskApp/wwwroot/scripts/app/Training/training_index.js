...
$scope.delete = function (removeitem) {
        ...
        if (ask == true) {
            $.post('/api/delete/training?id=' + removeitem.community_training_id).success(function (value) {
                $scope.loading = false;
    ...
    $scope.syncPost = function () {
        ...
        if ($scope.list_of_selected_items.length > 0) {
            ...
            for (i = 0; i < $scope.list_of_selected_items.length; i++) {
                var url = '/api/offline/v1/trainins/post/selected_items_to_sync?community_training_id='
                        + items_selected[i].community_training_id + "&push_status_id=" + 5;
                $scope.isAddingItem = true;

                $http.post(url).success(function (data) {
                    $scope.isAddingItem = false;
                    //inner http.post -- actual sync:
                    $http.post('/sync/post/trainings?username=' + username + "&password=" + password + "&record_id=").success(function (data) {
                        $scope.false = true;
                        $scope.isSearching = false;
                        location.reload();
                        $mdDialog.cancel();
