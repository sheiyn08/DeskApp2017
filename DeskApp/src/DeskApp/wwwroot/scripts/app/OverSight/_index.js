...
.then(function (response) { $scope.fund_source_id_options = response.data; });
    ...
    $http.get('/api/table_name?name=oversight_committee')
    ...
    $scope.search = function (page) {
        ...
        if (($scope.data.filter_by_recent_edit) && (!$scope.data.filter_by_recent_add)) {
            $.post('/api/offline/v1/oversight_committee/get_recently_edited', $scope.data).success(function (value) {
                $scope.loading = false;
                ...
                angular.forEach($scope.Items, function (record) {
                    $http.get('/api/exists/record_attachment?id=' + record.oversight_committee_id)
            ...
            $.post('/api/offline/v1/oversight_committee/get_recently_added', $scope.data).success(function (value) {
                $scope.loading = false;
                $scope.TotalUnAuthorized = value.TotalUnAuthorized;
                $scope.TotalSync = value.TotalSync;
                $scope.page = value.Page;
            ...
            $.post('/api/offline/v1/oversight_committee/get_recently_edited_and_added', $scope.data).success(function (value) {
                $scope.loading = false;
                $scope.TotalUnAuthorized = value.TotalUnAuthorized;
                $scope.TotalSync = value.TotalSync;
                $scope.page = value.Page;
            ...
            $.post('/api/offline/v1/oversight_committee/get_dto', $scope.data).success(function (value) {
                $scope.loading = false;
                $scope.TotalUnAuthorized = value.TotalUnAuthorized;
                $scope.TotalSync = value.TotalSync;
                $scope.page = value.Page;
    ...
    $scope.createNew = function () {
        window.location.href = "/Entry/Oversight";
    };
    ...
    $scope.syncPost = function () {
        ...
        if ($scope.list_of_selected_items.length > 0) {
            ...
            for (i = 0; i < $scope.list_of_selected_items.length; i++) {
                var url = '/api/offline/v1/oversight/post/selected_items_to_sync?oversight_committee_id='
                        + items_selected[i].oversight_committee_id + "&push_status_id=" + 5;
                $scope.isAddingItem = true;

                $http.post(url).success(function (data) {
                    $scope.isAddingItem = false;
                    //inner http.post -- actual sync:      
                    $http.post('/sync/post/oversight_committee?username=' + username + "&password=" + password + "&record_id=").success(function (data) {
                        $scope.needToConfirm = false;
                        $scope.false = true;
                        $scope.isSearching = false;
                        location.reload();
