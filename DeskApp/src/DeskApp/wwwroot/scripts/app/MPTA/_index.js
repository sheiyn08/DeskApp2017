...
.then(function (response) { $scope.enrollment_id_options = response.data; });
    ...
    $scope.delete = function (removeitem) {
        ...
        if (ask == true) {
            $.post('/api/delete/municipal_pta?id=' + removeitem.municipal_pta_id).success(function (value) {
                $scope.loading = false;
    ...
    $http.get('/api/table_name?name=municipal_pta')
    ...
    $scope.search = function (page) {
        ...
        if (($scope.data.filter_by_recent_edit) && (!$scope.data.filter_by_recent_add)) {
            $.post('/api/offline/v1/MPTA/get_recently_edited', $scope.data).success(function (value) {
                $scope.loading = false;
                ...
                angular.forEach($scope.Items, function (record) {
                    $http.get('/api/exists/record_attachment?id=' + record.municipal_pta_id)
            ...
            $.post('/api/offline/v1/MPTA/get_recently_added', $scope.data).success(function (value) {
                $scope.loading = false;
            ...
            $.post('/api/offline/v1/MPTA/get_recently_edited_and_added', $scope.data).success(function (value) {
                $scope.loading = false;
            ...
            $.post('/api/offline/v1/MPTA/get_dto', $scope.data).success(function (value) {
                $scope.loading = false;
    ...
    $scope.createNew = function () {
        window.location.href = "/Entry/MPTA";
    };
...
.then(function (response) { $scope.all_city_code_options = response.data; });
    ...
    $scope.syncGet = function () {
        ...
        $http.post('/sync/get/MPTA?username=' + username + "&password=" + password + "&city_code=" + $scope.city_code  +"&record_id=").success(function (data) {
            $scope.false = true;
            $mdDialog.cancel();
            location.reload();
            $scope.isSearching = false;
    ...
    $scope.syncPost = function () {
        ...
        if ($scope.list_of_selected_items.length > 0) {
            ...
            for (i = 0; i < $scope.list_of_selected_items.length; i++) {
                var url = '/api/offline/v1/municipal_pta/post/selected_items_to_sync?municipal_pta_id='
                        + items_selected[i].municipal_pta_id + "&push_status_id=" + 5;
                $scope.isAddingItem = true;

                $http.post(url).success(function (data) {
                    $scope.isAddingItem = false;
                    //inner http.post -- actual sync:
                    $http.post('/sync/post/MPTA?username=' + username + "&password=" + password + "&record_id=").success(function (data) {
                        $scope.false = true;
                        $scope.isSearching = false;
                        location.reload();
                        $mdDialog.cancel();
