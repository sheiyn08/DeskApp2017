...
angular.module('MyApp', ['ngMaterial', 'ngMessages', 'material.svgAssetsCache', 'pims-pager']).controller('AppCtrl', function ($scope, $http, $mdDialog, $mdMedia) {
    ...
    $scope.checkAll = function () {
        ...
        angular.forEach($scope.Items, function (item) {
            item.is_item_selected = $scope.data.check_all;
        });
...
.then(function (response) { $scope.grs_intake_officer_id_options = response.data; });
    ...
    $scope.delete = function (removeitem) {
        ...
        if (ask == true) {
            $.post('/api/delete/community_organization?id=' + removeitem.community_organization_id).success(function (value) {
                $scope.loading = false;
    ...
    $scope.search = function (page) {
        ...
        if (($scope.data.filter_by_recent_edit) && (!$scope.data.filter_by_recent_add)) {
            $.post('/api/offline/v1/community_organization/get_recently_edited', $scope.data).success(function (value) {
                $scope.loading = false;
                ...
                angular.forEach($scope.Items, function (record) {
                    $http.get('/api/exists/record_attachment?id=' + record.community_organization_id)
            ...
            $.post('/api/offline/v1/community_organization/get_recently_added', $scope.data).success(function (value) {
                $scope.loading = false;
                $scope.TotalUnAuthorized = value.TotalUnAuthorized;
                $scope.TotalSync = value.TotalSync;
                $scope.page = value.Page;
            ...
            $.post('/api/offline/v1/community_organization/get_recently_edited_and_added', $scope.data).success(function (value) {
                $scope.loading = false;
                $scope.TotalUnAuthorized = value.TotalUnAuthorized;
                $scope.TotalSync = value.TotalSync;
                $scope.page = value.Page;
            ...
            $.post('/api/offline/v1/community_organization/get_dto', $scope.data).success(function (value) {
                $scope.loading = false;
                $scope.TotalUnAuthorized = value.TotalUnAuthorized;
                $scope.TotalSync = value.TotalSync;
                $scope.page = value.Page;
    ...
    $scope.search();
     ...
     $http.get('/api/table_name?name=community_organization')
   .then(function (response) {
    ...
    $scope.createNew = function () {
        ...
        window.location.href = "/Entry/Organizations";
    }
...
.then(function (response) { $scope.all_city_code_options = response.data; });
    ...
    $scope.syncGet = function () {
        ...
        $http.post('/sync/get/community_organization?username=' + username + "&password=" + password + "&city_code=" + $scope.city_code  +"&record_id=" 
    ...
    $scope.syncPost = function () {
        ...
        if ($scope.list_of_selected_items.length > 0) {
            ...
            for (i = 0; i < $scope.list_of_selected_items.length; i++) {
                var url = '/api/offline/v1/community_organizations/post/selected_items_to_sync?community_organization_id='
                        + items_selected[i].community_organization_id + "&push_status_id=" + 5;
                $scope.isAddingItem = true;

                $http.post(url).success(function (data) {
                    $scope.isAddingItem = false;
                    //inner http.post -- actual sync:
                    $http.post('/sync/post/community_organization?username=' + username + "&password=" + password + "&record_id=").success(function (data) {
                        $scope.needToConfirm = false;
                        $scope.false = true;
                        $scope.isSearching = false;
                        location.reload();
