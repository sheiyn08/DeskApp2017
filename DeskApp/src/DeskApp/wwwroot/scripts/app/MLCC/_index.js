...
angular.module('MyApp', ['ngMaterial', 'ngMessages', 'material.svgAssetsCache', 'pims-pager']).controller('AppCtrl', function ($scope, $http, $mdDialog, $mdMedia) {
    ...
    $scope.delete = function (removeitem) {
        ...
        if (ask === true) {
            $.post('/api/delete/municipal_lcc?id=' + removeitem.municipal_lcc_id).success(function (value) {
                $scope.loading = false;
    ...
    $http.get('/api/lib_enrollment').then(function (response) {
    ...
    $http.get('/api/table_name?name=municipal_lcc').then(function (response) {

        $scope.table_name_id = response.data;
        $http.get('/api/report_list?id=' + $scope.table_name_id).then(function (response) {
    ...
    $scope.search = function (page) {
        ...
        if (($scope.data.filter_by_recent_edit) && (!$scope.data.filter_by_recent_add)) {
            $.post('/api/offline/v1/mlcc/get_recently_edited', $scope.data).success(function (value) {
                $scope.loading = false;
            ...
            $.post('/api/offline/v1/mlcc/get_recently_added', $scope.data).success(function (value) {
                $scope.loading = false;
                $scope.TotalSync = value.TotalSync;
                $scope.TotalUnAuthorized = value.TotalUnAuthorized;
                $scope.page = value.Page;
            ...
            $.post('/api/offline/v1/mlcc/get_recently_edited_and_added', $scope.data).success(function (value) {
                $scope.loading = false;
                $scope.TotalSync = value.TotalSync;
                $scope.TotalUnAuthorized = value.TotalUnAuthorized;
                $scope.page = value.Page;
            ...
            $.post('/api/offline/v1/mlcc/get_dto', $scope.data).success(function (value) {
                $scope.loading = false;
                $scope.TotalSync = value.TotalSync;
                $scope.TotalUnAuthorized = value.TotalUnAuthorized;
                $scope.page = value.Page;
    ...
    $scope.createNew = function () {
         ...
         window.location.href = "/Entry/MLCC";
    }
...
function DialogController($scope, $mdDialog, $http, items_selected) {
    ...
    $http.get('/api/all/cities').then(function (response) {
        $scope.all_city_code_options = response.data;
    });
    ...
    $scope.syncPost = function () {
        ...
        if ($scope.list_of_selected_items.length > 0) {
            ...
            for (i = 0; i < $scope.list_of_selected_items.length; i++) {
                var url = '/api/offline/v1/municipal_lcc/post/selected_items_to_sync?municipal_lcc_id='
                        + items_selected[i].municipal_lcc_id + "&push_status_id=" + 5;
                $scope.isAddingItem = true;

                $http.post(url).success(function (data) {
                    $scope.isAddingItem = false;
                    //inner http.post -- actual sync:
                    $http.post('/sync/post/mlcc?username=' + username + "&password=" + password + "&record_id=").success(function (data) {
                        $scope.needToConfirm = false;
                        $scope.false = true;
                        $scope.isSearching = false;
                        location.reload();
