...
.then(function (response) { $scope.enrollment_id_options = response.data; });
    ...
    $scope.search = function (page) {
        ...
        if (($scope.data.filter_by_recent_edit) && (!$scope.data.filter_by_recent_add)) {
            ...
            $.post('/api/offline/v1/ceac_list/get_recently_edited', $scope.data).success(function (value) {
                $scope.loading = false;
            ...
            $.post('/api/offline/v1/ceac_list/get_recently_added', $scope.data).success(function (value) {
                $scope.loading = false;
            ...
            $.post('/api/offline/v1/ceac_list/get_recently_edited_and_added', $scope.data).success(function (value) {
                $scope.loading = false;
    ...
    $http.get('/api/table_name?name=ceac_municipality')
    ...
    $scope.syncPost = function () {
        ...
        if ($scope.list_of_selected_items.length > 0) {
            ...
            for (i = 0; i < $scope.list_of_selected_items.length; i++) {
                var url = '/api/offline/v1/ceac_tracking/post/selected_items_to_sync?ceac_list_id='
                        + items_selected[i].ceac_list_id + "&push_status_id=" + 5;
                $scope.isAddingItem = true;

                $http.post(url).success(function (data) {
                    $scope.isAddingItem = false;
                    //inner http.post -- actual sync:
                    $http.post('/sync/post/ceac?username=' + username + "&password=" + password + "&record_id=").success(function (data) {
                        $scope.needToConfirm = false;
                        $scope.false = true;
                        $scope.isSearching = false;
                        location.reload();
